...
public class ShortestRouteCalculator extends RouteCalculatorHelper {

    public static List<Route> getMinimumDistanceWithOneTransfer(List<Route> allRoutes, String countryFrom, String countryTo) {
        List<String> vertices = getListOfCountries(allRoutes);
        int n = vertices.size();
        Double[][] matrix = getGraphAdjacencyDistanceMatrix(vertices, allRoutes);
        Route r;
                    ...
                    if (j != k && matrix[j][i] != -1 && matrix[i][k] != -1) {
                        ...
                        if (matrix[j][k] == -1) {
                            if (arrayOfArraysOfRoutes[j][k].size() == 0) {
                                r = getRouteWithMinimumDistance(vertices.get(j), vertices.get(i), allRoutes);
                                if (r != null) arrayOfArraysOfRoutes[j][k].add(r);
                                r = getRouteWithMinimumDistance(vertices.get(i), vertices.get(k), allRoutes);
                                if (r != null) arrayOfArraysOfRoutes[j][k].add(r);
                            ...
                            if (matrix[j][k] < matrixSum) {
                                if (arrayOfArraysOfRoutes[j][k].size() == 0) {
                                    r = getRouteWithMinimumDistance(vertices.get(j), vertices.get(k), allRoutes);
                                    if (r != null) arrayOfArraysOfRoutes[j][k].add(r);
                                } else if (arrayOfArraysOfRoutes[j][k].size() > 0 &&
    ...
    public static List<Route> getMinimumDistanceWithTwoTransfers(List<Route> allRoutes, String countryFrom, String countryTo) {
        List<String> vertices = getListOfCountries(allRoutes);
        int n = vertices.size();
        Double[][] matrix = getGraphAdjacencyDistanceMatrix(vertices, allRoutes);
        Route r;
                        ...
                        if (k != m && matrix[k][j] != -1 && matrix[j][i] != -1 && matrix[i][m] != -1) {
                            ...
                            if (matrix[k][m] == -1) {
                                if (arrayOfArraysOfRoutes[k][m].size() == 0) {
                                    r = getRouteWithMinimumDistance(vertices.get(k), vertices.get(j), allRoutes);
                                    if (r != null) arrayOfArraysOfRoutes[k][m].add(r);
                                    r = getRouteWithMinimumDistance(vertices.get(j), vertices.get(i), allRoutes);
                                    if (r != null) arrayOfArraysOfRoutes[k][m].add(r);
                                    r = getRouteWithMinimumDistance(vertices.get(i), vertices.get(m), allRoutes);
                                    if (r != null) arrayOfArraysOfRoutes[k][m].add(r);
                                ...
                                if (matrix[k][m] < matrixSum) {
                                    if (arrayOfArraysOfRoutes[k][m].size() == 0) {
                                        r = getRouteWithMinimumDistance(vertices.get(k), vertices.get(m), allRoutes);
                                        if (r != null) arrayOfArraysOfRoutes[k][m].add(r);
                                    } else if (arrayOfArraysOfRoutes[k][m].size() > 0 &&
                                            arrayOfArraysOfRoutes[k][m].stream().mapToDouble(Route::getDistance).sum() > matrix[k][m]) {
                                        arrayOfArraysOfRoutes[k][m] = new ArrayList<>();
                                        r = getRouteWithMinimumDistance(vertices.get(k), vertices.get(m), allRoutes);
                                        if (r != null) arrayOfArraysOfRoutes[k][m].add(r);
                                    }
    ...
    public static List<Route> getMinimumDistanceWithThreeTransfers(List<Route> allRoutes, String countryFrom, String countryTo) {
        List<String> vertices = getListOfCountries(allRoutes);
        int n = vertices.size();
        Double[][] matrix = getGraphAdjacencyDistanceMatrix(vertices, allRoutes);
        Route r;
                            ...
                            if (m != a && matrix[m][k] != -1 && matrix[k][j] != -1 && matrix[j][i] != -1 && matrix[i][a] != -1) {
                                ...
                                if (matrix[m][a] == -1) {
                                    if (arrayOfArraysOfRoutes[m][a].size() == 0) {
                                        r = getRouteWithMinimumDistance(vertices.get(m), vertices.get(k), allRoutes);
                                        if (r != null) arrayOfArraysOfRoutes[m][a].add(r);
                                        r = getRouteWithMinimumDistance(vertices.get(k), vertices.get(j), allRoutes);
                                        if (r != null) arrayOfArraysOfRoutes[m][a].add(r);
                                        r = getRouteWithMinimumDistance(vertices.get(j), vertices.get(i), allRoutes);
                                        if (r != null) arrayOfArraysOfRoutes[m][a].add(r);
                                        r = getRouteWithMinimumDistance(vertices.get(i), vertices.get(a), allRoutes);
                                        if (r != null) arrayOfArraysOfRoutes[m][a].add(r);
                                    ...
                                    if (matrix[m][a] < matrixSum) {
                                        if (arrayOfArraysOfRoutes[m][a].size() == 0) {
                                            r = getRouteWithMinimumDistance(vertices.get(m), vertices.get(a), allRoutes);
                                            if (r != null) arrayOfArraysOfRoutes[m][a].add(r);
                                        } else if (arrayOfArraysOfRoutes[m][a].size() > 0 &&
                                                arrayOfArraysOfRoutes[m][a].stream().mapToDouble(Route::getDistance).sum() > matrix[m][a]) {
                                            arrayOfArraysOfRoutes[m][a] = new ArrayList<>();
                                            r = getRouteWithMinimumDistance(vertices.get(m), vertices.get(a), allRoutes);
                                            if (r != null) arrayOfArraysOfRoutes[m][a].add(r);
                                        }
